  ...
  return (
    <main className="min-h-screen w-full bg-white px-4 py-8 text-slate-900 sm:px-6 lg:px-8">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4 border-b border-blue-100 pb-4">
        <div>
          <h1 className="text-3xl font-bold text-blue-950">Cichlid Explorer</h1>
      ...
      <div className="mb-6 flex w-full flex-wrap items-center gap-3">
        ...
        <input
          type="number"
          className="w-40 rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
        ...
        <div className="ml-auto flex items-center gap-3">
          <button
            className="rounded-lg border border-blue-200 px-4 py-2 text-sm font-medium text-blue-700 hover:bg-blue-50 disabled:opacity-40"
            disabled={page === 1}
            onClick={() => setPage((p) => p - 1)}
          >
            Previous
          </button>
          <span className="text-sm text-blue-700">Page {page} of {totalPages}</span>
          <button
            className="rounded-lg border border-blue-200 px-4 py-2 text-sm font-medium text-blue-700 hover:bg-blue-50 disabled:opacity-40"
      ...
      {loading ? (
        <p className="py-12 text-center text-blue-300">Loading...</p>
      ) : (
